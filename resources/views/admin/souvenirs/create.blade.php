<x-admin-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-2">
            ...
            <h2 class="text-2xl font-semibold text-slate-900 dark:text-white">{{ __('Tambah Souvenir Baru') }}</h2>
        </div>
    </x-slot>
        ...
        <form action="{{ route('admin.souvenirs.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid gap-6 md:grid-cols-2">
                ...
                <div>
                    ...
                    <x-ui.input type="number" name="price" value="{{ old('price') }}" required />
                </div>
                <div>
                    <x-ui.label value="{{ __('Stok') }}" />
                    <x-ui.input type="number" name="stock" value="{{ old('stock') }}" required />
                </div>
                <div>
