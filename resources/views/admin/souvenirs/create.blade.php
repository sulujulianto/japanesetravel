<x-admin-layout>
    <x-slot name="header">
        <div class="rounded-2xl border border-[#E7E3DC] bg-white p-5 sm:p-6 dark:border-[#2A333D] dark:bg-[#161B22]">
            ...
            <h1 class="mt-2 text-2xl font-semibold tracking-tight text-[#1F2937] dark:text-[#F4F1ED] sm:text-3xl">{{ __('Tambah Souvenir Baru') }}</h1>
            <p class="mt-2 max-w-2xl text-sm leading-6 text-[#526071] dark:text-[#AEB8C7]">{{ __('Lengkapi konten bilingual, harga, stok, dan gambar produk untuk katalog toko.') }}</p>
        </div>
    </x-slot>
    ...
    <form action="{{ route('admin.souvenirs.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        ...
        <section class="{{ $sectionClass }}" aria-labelledby="souvenir-commerce">
            <div class="border-b border-[#E7E3DC] pb-4 dark:border-[#2A333D]">
                ...
                <p class="mt-1 text-sm text-[#526071] dark:text-[#AEB8C7]">{{ __('Harga dan stok digunakan langsung oleh alur toko dan keranjang.') }}</p>
            </div>
            <div class="mt-5 grid gap-5 md:grid-cols-2">
                <div>
                    <label for="price" class="{{ $labelClass }}">{{ __('Harga') }}</label>
                    <input id="price" type="number" name="price" value="{{ old('price') }}" min="0" step="any" required class="{{ $inputClass }}">
                ...
                <div>
                    ...
                    <input id="stock" type="number" name="stock" value="{{ old('stock') }}" min="0" step="1" required class="{{ $inputClass }}">
        ...
        <section class="{{ $sectionClass }}" aria-labelledby="souvenir-media">
            <div class="border-b border-[#E7E3DC] pb-4 dark:border-[#2A333D]">
                ...
                <p class="mt-1 text-sm text-[#526071] dark:text-[#AEB8C7]">{{ __('Gunakan gambar produk yang jelas dengan komposisi sederhana.') }}</p>
            </div>
