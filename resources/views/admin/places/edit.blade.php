<x-admin-layout>
    <x-slot name="header">
        <div class="rounded-2xl border border-[#E7E3DC] bg-white p-5 sm:p-6 dark:border-[#2A333D] dark:bg-[#161B22]">
            ...
            <h1 class="mt-2 text-2xl font-semibold tracking-tight text-[#1F2937] dark:text-[#F4F1ED] sm:text-3xl">{{ __('Edit Destinasi') }}</h1>
            <p class="mt-2 max-w-2xl text-sm leading-6 text-[#526071] dark:text-[#AEB8C7]">{{ __('Perbarui konten dan informasi kunjungan tanpa mengubah struktur katalog destinasi.') }}</p>
        </div>
    </x-slot>
        ...
        <section class="{{ $sectionClass }}" aria-labelledby="place-id-content">
            ...
            <div class="mt-5 grid gap-5">
                <div>
                    ...
                    <input id="name_id" name="name_id" value="{{ old('name_id', $place->getTranslation('name', 'id')) }}" required class="{{ $inputClass }}">
                    @error('name_id')<p class="{{ $errorClass }}">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label for="description_id" class="{{ $labelClass }}">{{ __('Deskripsi (ID)') }}</label>
                    <textarea id="description_id" name="description_id" rows="6" class="{{ $inputClass }}">{{ old('description_id', $place->getTranslation('description', 'id')) }}</textarea>
        ...
        <section class="{{ $sectionClass }}" aria-labelledby="place-en-content">
            ...
            <div class="mt-5 grid gap-5">
                <div>
                    ...
                    <input id="name_en" name="name_en" value="{{ old('name_en', $place->getTranslation('name', 'en')) }}" required class="{{ $inputClass }}">
                    @error('name_en')<p class="{{ $errorClass }}">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label for="description_en" class="{{ $labelClass }}">{{ __('Deskripsi (EN)') }}</label>
                    <textarea id="description_en" name="description_en" rows="6" class="{{ $inputClass }}">{{ old('description_en', $place->getTranslation('description', 'en')) }}</textarea>
        ...
        <section class="{{ $sectionClass }}" aria-labelledby="place-visit-information">
            ...
            <div class="mt-5 grid gap-5 md:grid-cols-2">
                <div class="md:col-span-2">
                    ...
                    <input id="address" name="address" value="{{ old('address', $place->address) }}" class="{{ $inputClass }}">
                ...
                <div class="md:col-span-2">
                    ...
                    <input id="facilities" name="facilities" value="{{ old('facilities', $place->facilities) }}" class="{{ $inputClass }}">
                ...
                <div>
                    ...
                    <input id="open_days" name="open_days" value="{{ old('open_days', $place->open_days) }}" class="{{ $inputClass }}">
                    @error('open_days')<p class="{{ $errorClass }}">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label for="open_hours" class="{{ $labelClass }}">{{ __('Jam Buka') }}</label>
                    <input id="open_hours" name="open_hours" value="{{ old('open_hours', $place->open_hours) }}" class="{{ $inputClass }}">
        ...
        <section class="{{ $sectionClass }}" aria-labelledby="place-media">
            ...
            <div class="mt-5 grid gap-5 lg:grid-cols-[12rem_minmax(0,1fr)] lg:items-start">
                <div>
                    ...
                    <div class="mt-2 aspect-[4/3] overflow-hidden rounded-xl border border-[#E7E3DC] bg-[#FAF8F3] dark:border-[#2A333D] dark:bg-[#0E1116]">
