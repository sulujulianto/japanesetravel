# JapanTravel Documentation

This directory separates recruiter-facing evidence, engineering documentation, operational checklists, and QA templates.

## Start Here

- [Project README](../README.md)
- [Case Study](case-study-japanese-travel.md)
- [Portfolio Readiness Checklist](portfolio-readiness-checklist.md)
- [Backend Technical Documentation](backend-technical-documentation.md)
- [Design System and Rebranding](design-system-and-rebranding.md)

## Deployment and Operations

- [Railway Staging Deployment Checklist](deployment-checklist.md)
- [Troubleshooting Case Studies](troubleshooting-case-studies.md)
- [SQL Inspection Examples](sql-inspection-examples.md)
- [Incident Report Template and Simulated Sample](incident-report-template.md)

## Portfolio Evidence

- [Demo Video Script](demo-video-script.md)
- [Screenshot Checklist](screenshots/README.md)
- [Testing Summary](testing-summary.md)
- [Project Workflow](project-workflow.md)
- [GitHub Issues Backlog](github-issues-backlog.md)
- [Ticket Documentation Samples](ticket-documentation-samples.md)

## QA

- [Test Cases](qa/test-cases-japanese-travel.md)
- [Manual QA Checklist](qa/manual-qa-checklist.md)
- [Regression Scenarios](qa/regression-test-scenarios.md)
- [Bug Report Samples](qa/bug-report-sample.md)
- [Test Execution Evidence Template](qa/test-execution-evidence.md)

## HTTP and Provider Checks

- [Postman/API Checking Guide](postman-api-checking.md)
- [Safe Postman Collection](postman/JapaneseTravel.postman_collection.json)

## Evidence Policy

- Automated test results may be stated only after running the command.
- Manual checks remain incomplete until tester/date/environment/evidence are recorded.
- External integrations remain owner actions until provider account evidence exists.
- Simulated tickets/incidents must remain labelled as simulated.
- Never commit credentials, cookies, personal data, or unredacted provider payloads.
